...
micro-scaling and NF4) for the latent, but our ablation
...
\texttt{\#[repr(C)]} structure whose total size is exactly
128~bytes \emph{with zero padding}. Its alignment is \emph{cache-line-aware}
and resolved at compile time from the target architecture
(\S\ref{sec:cachemap}). Table~\ref{tab:tile} gives the layout.

\begin{table}[t]
\centering
\caption{The \texttt{SciRustSlhaTile} layout ($\dc=128$, $\ds=256$).
Total $= 64+32+32 = 128$ bytes; \texttt{align\_of} is $128$ on AArch64
and $64$ elsewhere (size is $128$ either way); verified by
test \texttt{tile\_is\_exactly\_128\_bytes\_zero\_padding} and by AArch64
cross-compilation.}
\label{tab:tile}
\small
\begin{tabular}{@{}llr l@{}}
...
Because cache-line width differs across our targets---64~bytes on
mainstream x86-64, 128~bytes on Jetson Thor-class AArch64
(Neoverse-V3AE)---we resolve the alignment attribute at compile time from
\texttt{target\_arch} via \texttt{cfg\_attr} (listing above):
\texttt{align(128)} on AArch64, so a tile is \emph{exactly one} cache line
and a single line fill on the LPDDR5X bus; \texttt{align(64)} elsewhere, so
a tile spans two lines. The \emph{size} is 128~bytes in both cases
(128 is a multiple of both alignments), so zero padding holds regardless of
target. No \texttt{build.rs} is needed because \texttt{target\_arch} is a
compile-time \texttt{cfg}; a build script would only help to probe the
\emph{host's} actual line size under \texttt{-C target-cpu=native}
(\S\ref{sec:future}). We cross-compile to
\texttt{aarch64-unknown-linux-gnu} in CI so the AArch64 layout and NEON
kernel are type-checked.
...
\begin{itemize}[leftmargin=1.4em,itemsep=2pt]
  ...
  \item \textbf{Zero dependencies.} The library has no runtime
    ...
    micro-benchmarks.
  \item \textbf{Tested equivalence.} 41 tests (unit, integration,
...
same 128-byte tile). We add an INT8 reference (coarse only, hypothetical
192-byte tile) to isolate bit-width (Table~\ref{tab:ablation}).
...
null-to-marginal ($0.884 \to 0.885$). Crucially, INT8 does \emph{not}
...
AVX-512. A NEON path exists and is cross-compile-verified but not timed
on this (x86-64) bench. SLHA v2 targets \emph{both} families---x86-64
servers and AArch64 edge devices (e.g.\ Jetson Thor AGX 128,
Neoverse-V3AE)---and the crate ships a portable \texttt{platform\_report}
bench kit (and a runner script) that detects features (AVX/NEON/SVE2),
confirms the cache-line-aware tile alignment, and measures
wall-clock throughput on whatever CPU runs it. The x86-64 figures above are
the server baseline; on-device AArch64 numbers are reproduced by running
the kit on the target rather than fabricated here.
...
\begin{itemize}[leftmargin=1.4em,itemsep=2pt]
  ...
  \item \textbf{No ARM timing.} The NEON path is compile-checked by
    cross-compilation only; SLHA's claims for ARM edge targets are
    therefore architectural, not yet measured.
...
\section{Future Work}
\label{sec:future}
Two of the hardware directions we had identified are now \emph{implemented}
and described above: the cache-line-aware alignment (\S\ref{sec:cachemap})
and the AVX-512 \textsc{vpopcntdq} residual reduction (\S\ref{sec:impl}).
The remaining directions are framed as proposals (not measured here).

\begin{enumerate}[leftmargin=1.6em,itemsep=2pt]
  \item \textbf{Scalable SIMD on ARM (Jetson Thor class).} The coarse dot is
    the hot term; an SVE2 path on Neoverse-V3AE could use \emph{scalable}
    vectors (adapting to the implementation's vector length) beyond fixed
    NEON. The residual popcount is already branchless and near-optimal at
    256 bits, so the lever is the dot, not the popcount. We deliberately
    keep this on the roadmap rather than in the shipped kernel for a
    concrete toolchain reason: SVE2 intrinsics and portable
    \texttt{std::simd} are nightly-gated, whereas the reference crate is
    intentionally stable and dependency-free. The NEON\,+\,\texttt{cnt}
    fallback remains correct meanwhile; SVE2 will land behind runtime
    vector-length detection once the intrinsics stabilize --- and requires
    ARM hardware to measure.
  \item \textbf{Host-adaptive alignment via \texttt{build.rs}.} The
    compile-time \texttt{cfg\_attr} alignment covers cross-compilation to a
    \emph{known} target. For native builds (\texttt{-C target-cpu=native})
    a build script could probe the host's \emph{actual} cache-line width and
    emit a custom \texttt{cfg} (e.g.\ \texttt{cache\_line\_128}), refining
    the ``AArch64 $\Rightarrow$ 128'' heuristic (not all AArch64 parts use
    128-byte lines). This is a portability refinement, not a correctness
    change.
  \item \textbf{Salient-outlier tiles (BiLLM-style).} Following
    ...
    not assumed. \emph{(ii)~Expected benefit:} our ablation
    (\S\ref{sec:ablation}) shows quantization is not the current bottleneck
    on this benchmark; BiLLM targets quantization-error outliers, a regime
    our synthetic generator does not yet model. The honest prerequisite is
    therefore to extend the benchmark with realistic activation outliers, so
    any salient-value gain can be \emph{demonstrated} before it is baked into
    the tile.
\end{enumerate}
...
The reference crate (\textsc{SciRust}), all benchmarks, and the 41 tests
